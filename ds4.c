...
static bool accelerator_cuda_q4_preload_tensor_name(const char *name) {
...
static bool accelerator_cache_model_tensors(ds4_backend backend, const ds4_model *m) {
    ...
    if (getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        ...
        for (uint64_t i = 0; i < m->n_tensors; i++) {
            const ds4_tensor *t = &m->tensors[i];
            if (t->bytes == 0 || t->ndim != 2) continue;
            if (t->abs_offset > m->size || t->bytes > m->size - t->abs_offset) return false;
            char name[128];
            snprintf(name, sizeof(name), "%.*s", (int)t->name.len, t->name.ptr);
            if (t->type == DS4_TENSOR_Q8_0) {
                if (!accelerator_cuda_q4_preload_tensor_name(name)) continue;
                char label[160];
                snprintf(label, sizeof(label), "q4:%s", name);
                if (ds4_gpu_cache_q4_0_range(m->map, m->size, t->abs_offset, t->bytes, t->dim[0], t->dim[1], label) == 0) {
                    fprintf(stderr, "ds4: accelerator failed to cache Q4 tensor %s\n", name);
                    return false;
                }
                q4_count++;
            } else if (t->type == DS4_TENSOR_F16 &&
                       (strstr(name, ".indexer.attn_q_b.weight") != NULL ||
        ...
        if (q4_count != 0 || q4f16_count != 0) {
            fprintf(stderr, "ds4: CUDA preloaded Q4 decode cache for %llu Q8_0 + %llu F16 tensors in %.3fs\n",
...
static ds4_gpu_tensor *layer_comp_cache_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_attn_comp_cache[il]) return g->layer_attn_comp_cache[il];
    if (g->layer_comp_cap[il] == 0) return NULL;
...
static ds4_gpu_tensor *layer_raw_cache_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_raw_cache[il]) return g->layer_raw_cache[il];
    if (g->raw_cap == 0) return NULL;
    g->layer_raw_cache[il] = ds4_gpu_tensor_alloc(
        (uint64_t)g->raw_cap * DS4_N_HEAD_DIM * sizeof(float));
    return g->layer_raw_cache[il];
}

static void layer_attn_state_dims(uint32_t il, uint64_t *out_width, uint64_t *out_rows) {
    const uint32_t ratio = ds4_layer_compress_ratio(il);
    if (ratio == 0) {
        *out_width = 0;
        *out_rows = 0;
    ...
    const uint32_t coff = ratio == 4 ? 2u : 1u;
    *out_width = (uint64_t)coff * DS4_N_HEAD_DIM;
    *out_rows  = (uint64_t)coff * ratio;
}

static ds4_gpu_tensor *layer_attn_state_kv_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_attn_state_kv[il]) return g->layer_attn_state_kv[il];
    uint64_t w = 0, r = 0;
    layer_attn_state_dims(il, &w, &r);
    if (w == 0 || r == 0) return NULL;
    g->layer_attn_state_kv[il] = ds4_gpu_tensor_alloc(w * r * sizeof(float));
    if (g->layer_attn_state_kv[il]) {
        (void)metal_tensor_fill_f32(g->layer_attn_state_kv[il], 0.0f, w * r);
        /* Symmetric visibility-lag fence with the index_* helpers
         * (see phase-3 commit d02a270): phase-2 happens to work
         * without this on this GB10 SKU, but the underlying
         * cudaMalloc-page-visibility issue is the same. */
        (void)ds4_gpu_synchronize();
    }
    return g->layer_attn_state_kv[il];
}

static ds4_gpu_tensor *layer_attn_state_score_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_attn_state_score[il]) return g->layer_attn_state_score[il];
    uint64_t w = 0, r = 0;
    layer_attn_state_dims(il, &w, &r);
    if (w == 0 || r == 0) return NULL;
    g->layer_attn_state_score[il] = ds4_gpu_tensor_alloc(w * r * sizeof(float));
    if (g->layer_attn_state_score[il]) {
        (void)metal_tensor_fill_f32(g->layer_attn_state_score[il], DS4_NEG_INF, w * r);
        (void)ds4_gpu_synchronize();
    }
    return g->layer_attn_state_score[il];
}

/* Indexer state buffers exist only for ratio==4 layers. */
static void layer_index_state_dims(uint32_t il, uint64_t *out_width, uint64_t *out_rows) {
    const uint32_t ratio = ds4_layer_compress_ratio(il);
    if (ratio != 4) {
        *out_width = 0;
        *out_rows = 0;
    ...
    *out_width = (uint64_t)coff * DS4_N_INDEXER_HEAD_DIM;
    *out_rows  = (uint64_t)coff * ratio;
}

static ds4_gpu_tensor *layer_index_comp_cache_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_index_comp_cache[il]) return g->layer_index_comp_cache[il];
    if (ds4_layer_compress_ratio(il) != 4) return NULL;
    if (g->layer_comp_cap[il] == 0) return NULL;
    g->layer_index_comp_cache[il] = ds4_gpu_tensor_alloc(
        (uint64_t)g->layer_comp_cap[il] * DS4_N_INDEXER_HEAD_DIM * sizeof(float));
    return g->layer_index_comp_cache[il];
}

static ds4_gpu_tensor *layer_index_state_kv_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_index_state_kv[il]) return g->layer_index_state_kv[il];
    uint64_t w = 0, r = 0;
    layer_index_state_dims(il, &w, &r);
    if (w == 0 || r == 0) return NULL;
    g->layer_index_state_kv[il] = ds4_gpu_tensor_alloc(w * r * sizeof(float));
    if (g->layer_index_state_kv[il]) {
        (void)metal_tensor_fill_f32(g->layer_index_state_kv[il], 0.0f, w * r);
        /* Sync: phase-3 saw post-fill garbage at pool=8 ctx=32K; force
         * the fill to complete before any consumer reads this buffer. */
        (void)ds4_gpu_synchronize();
    }
    return g->layer_index_state_kv[il];
}

static ds4_gpu_tensor *layer_index_state_score_ensure(ds4_gpu_graph *g, uint32_t il) {
    if (il >= DS4_N_LAYER) return NULL;
    if (g->layer_index_state_score[il]) return g->layer_index_state_score[il];
    uint64_t w = 0, r = 0;
    layer_index_state_dims(il, &w, &r);
    if (w == 0 || r == 0) return NULL;
    g->layer_index_state_score[il] = ds4_gpu_tensor_alloc(w * r * sizeof(float));
    if (g->layer_index_state_score[il]) {
        (void)metal_tensor_fill_f32(g->layer_index_state_score[il], DS4_NEG_INF, w * r);
        (void)ds4_gpu_synchronize();
    }
    return g->layer_index_state_score[il];
}
...
static bool metal_graph_alloc_raw_cap(
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        if (ratio != 0) {
            ...
            /* state_kv / state_score fill is now done at ensure-time. */
            (void)state_init_ok;
    ...
    for (uint32_t il = 0; layer_cache_ok && il < DS4_N_LAYER; il++) {
        ...
        if (layer_cache_ok && ratio != 0) {
            layer_cache_ok = g->layer_comp_cap[il] != 0u &&
                             (!enable_mtp ||
        ...
        if (layer_cache_ok && ratio == 4) {
            /* Lazy-alloc: layer_index_* are NULL until first use. */
            layer_cache_ok = g->layer_comp_cap[il] != 0u &&
                             (!enable_mtp ||
...
static bool metal_graph_capture_prefix1_attn_state(ds4_gpu_graph *g, uint32_t il) {
    ...
    const uint64_t bytes = ds4_gpu_tensor_bytes(layer_attn_state_kv_ensure(g, il));
    g->spec_prefix1_n_comp[il] = g->layer_n_comp[il];
    return ds4_gpu_tensor_copy(g->spec_prefix1_attn_state_kv[il], 0,
                                 layer_attn_state_kv_ensure(g, il), 0, bytes) != 0 &&
           ds4_gpu_tensor_copy(g->spec_prefix1_attn_state_score[il], 0,
                                 layer_attn_state_score_ensure(g, il), 0, bytes) != 0;
}
 ...
 * PRE_C splits into PRE_C1 (head_norm + rope + kv_store + compressor +
 * attention: writes g->heads), PRE_C2 (attn_output: writes g->attn_out
 * and, when fused, also g->after_attn_hc), and PRE_C3 (steering +
 * hc_expand + FFN HC pre + router + MoE).  Each opt-in env var
 * substitutes its corresponding sub-phase:
 *   DS4_CUDA_BATCHED_QKV         -> PRE_A2  via metal_graph_encode_qkv_batched
 *   DS4_CUDA_BATCHED_Q_B         -> PRE_B   via metal_graph_encode_q_b_batched
 *   DS4_CUDA_BATCHED_ATTN_OUTPUT -> PRE_C2  via metal_graph_encode_attn_output_batched
 ...
 * The legacy PRE_C1 bit is reserved for the pre-attention sub-phase to
 * preserve numeric continuity with prior masks. */
#define DS4_DECODE_LAYER_PRE_C1_PRE_ATTN   8u
#define DS4_DECODE_LAYER_PRE_C1_ATTN       512u
#define DS4_DECODE_LAYER_PRE_C1_POST_ATTN  1024u
#define DS4_DECODE_LAYER_PRE_C1  \
    (DS4_DECODE_LAYER_PRE_C1_PRE_ATTN | DS4_DECODE_LAYER_PRE_C1_ATTN | DS4_DECODE_LAYER_PRE_C1_POST_ATTN)
#define DS4_DECODE_LAYER_PRE_C2  16u
#define DS4_DECODE_LAYER_PRE_C3_ROUTER  32u   /* HC expand + ffn_norm + router select */
#define DS4_DECODE_LAYER_PRE_C3_MOE     256u  /* routed MoE (6 experts) */
#define DS4_DECODE_LAYER_PRE_C3  \
    (DS4_DECODE_LAYER_PRE_C3_ROUTER | DS4_DECODE_LAYER_PRE_C3_MOE)
#define DS4_DECODE_LAYER_PRE_C   \
    (DS4_DECODE_LAYER_PRE_C1 | DS4_DECODE_LAYER_PRE_C2 | DS4_DECODE_LAYER_PRE_C3)
#define DS4_DECODE_LAYER_PRE     \
    (DS4_DECODE_LAYER_PRE_A | DS4_DECODE_LAYER_PRE_B | DS4_DECODE_LAYER_PRE_C)
#define DS4_DECODE_LAYER_SHARED  64u
#define DS4_DECODE_LAYER_POST    128u
...
#define DS4_METAL_PROFILE_DECODE_STAGE(name) do { \
    ...
    /* fuse_hc_norm is needed in both PRE_A (HC pre/norm fusion) and PRE_C
     * (the !fuse_hc_norm branch handles the kv norm); hoist out of PRE_A
     * scope so the PRE_C reference compiles after the phase split. */
    ...
    if (qkv_rms_fused) {
        if (ok && !ds4_gpu_matmul_q8_0_dual_tensor(g->qr, g->kv_raw,
                                                    model->map, model->size,
    ...
    if (phases & DS4_DECODE_LAYER_PRE_B) {
    ...
    /* === PRE_C1: head_rms_norm + rope + kv path tail + kv_store +
     ...
     *   PRE_C1_POST_ATTN -- rope back on heads
     * Variables shared across sub-phases (n_comp, comp_cache,
     * comp_selected, n_selected) live at the outer PRE_C1 scope so they
     * flow correctly when PRE_C1 (composite) is requested. === */
    if (phases & DS4_DECODE_LAYER_PRE_C1) {
    uint32_t n_comp = 0;
    ...
    if (phases & DS4_DECODE_LAYER_PRE_C1_PRE_ATTN) {
    ...
    if (ok && compressed) {
        ...
        if (ok) ok = ds4_gpu_compressor_update_tensor(g->comp_kv_cur,
                                                        g->comp_sc_cur,
                                                        layer_attn_state_kv_ensure(g, il),
                                                        layer_attn_state_score_ensure(g, il),
                                                        layer_comp_cache_ensure(g, il),
                                                        model->map,
                                                        model->size,
        ...
        if (ok && ratio == 4) {
            ...
            if (ok) ok = ds4_gpu_compressor_update_tensor(g->comp_kv_cur,
                                                            g->comp_sc_cur,
                                                            layer_index_state_kv_ensure(g, il),
                                                            layer_index_state_score_ensure(g, il),
                                                            layer_index_comp_cache_ensure(g, il),
                                                            model->map,
                                                            model->size,
        ...
        comp_cache = layer_comp_cache_ensure(g, il);
    }
    DS4_METAL_PROFILE_DECODE_STAGE("compressor_indexer");
    } /* === end PRE_C1_PRE_ATTN === */

    if (phases & DS4_DECODE_LAYER_PRE_C1_ATTN) {
    if (ok) {
    ...
    if (phases & DS4_DECODE_LAYER_PRE_C1_POST_ATTN) {
    ...
    } /* === end PRE_C1_POST_ATTN === */
    } /* === end PRE_C1 (composite) === */
    /* fuse_attn_out_hc is needed in both PRE_C2 (chooses the fused
     * output_a + grouped + output_b + hc_expand kernel) and PRE_C3
     * (skips a separate hc_expand when the fused path already wrote
     * after_attn_hc).  Hoist out of the C2 scope so the C3 reference
     * after the split below compiles. */
    const bool fuse_attn_out_hc =
        !metal_graph_directional_steering_attn_enabled(g) &&
        !metal_graph_use_reference_attn_out_hc();
    /* === PRE_C2: attn_output.  Reads g->heads, writes g->attn_out (and
     * also g->after_attn_hc when fuse_attn_out_hc is on). === */
    if (phases & DS4_DECODE_LAYER_PRE_C2) {
    ...
    /* === PRE_C3_ROUTER: directional steering + hc_expand (when not fused) +
     * FFN HC pre + router select.  Reads g->attn_out (and g->after_attn_hc
     * when fused), writes g->ffn_norm + g->router_selected/weights. === */
    if (phases & DS4_DECODE_LAYER_PRE_C3_ROUTER) {
    if (ok && metal_graph_directional_steering_attn_enabled(g)) {
        ok = metal_graph_apply_directional_steering_attn(g, g->attn_out, il, 1);
    }
    /* PRE_C3 needs to run a separate hc_expand whenever PRE_C2's fused
     * path didn't already write g->after_attn_hc.  That's two cases:
     * (a) reference / directional-steering paths force the non-fused
     *     branch in PRE_C2 (!fuse_attn_out_hc); and
     * (b) PRE_C2 was substituted externally (e.g.
     *     metal_graph_encode_attn_output_batched), in which case PRE_C2
     *     is not in `phases` and only filled g->attn_out across N
     *     sessions -- no after_attn_hc was written. */
    const bool need_separate_hc_expand =
        !fuse_attn_out_hc || !(phases & DS4_DECODE_LAYER_PRE_C2);
    if (ok && need_separate_hc_expand) {
    ...
    /* === PRE_C3_MOE: routed MoE.  Reads g->ffn_norm + g->router_selected/
     * weights, writes g->routed_out.  Batched-decode can substitute this
     * with metal_graph_encode_routed_moe_batched. === */
    if (phases & DS4_DECODE_LAYER_PRE_C3_MOE) {
    ...
    } /* === end PRE_C3_MOE (= end of original PRE phase) === */
...
#define DS4_METAL_PROFILE_Q_STAGE(name) do { \
     ...
     */
    if (ok && zero_prefix) ok = ds4_gpu_store_raw_kv_batch_tensor(layer_raw_cache_ensure(g, il),
                                                                    g->batch_kv,
                                                                    g->raw_cap,
                                                                    pos0,
                                                                    n_tokens,
    ...
    if (ok && raw_batch_attention) {
        ...
        const uint32_t raw_start = metal_graph_raw_start_for_span(g,
                                                                  pos0 + n_tokens - 1u,
                                                                  n_raw);
        ok = ds4_gpu_store_raw_kv_batch_tensor(layer_raw_cache_ensure(g, il),
                                                 g->batch_kv,
                                                 g->raw_cap,
                                                 pos0,
                                                 n_tokens,
        ...
        if (zero_prefix) {
            ...
            if (ok) {
                ok = ds4_gpu_compressor_prefill_tensor(layer_comp_cache_ensure(g, il),
                                                         layer_attn_state_kv_ensure(g, il),
                                                         layer_attn_state_score_ensure(g, il),
                                                         g->batch_comp_kv,
                                                         g->batch_comp_sc,
            ...
            if (aligned_chunk) {
                ...
                if (ok) {
                    comp_view = ds4_gpu_tensor_view(layer_comp_cache_ensure(g, il),
        ...
        if (ok && ratio == 4) {
            ...
            if (zero_prefix) {
                ...
                if (ok) {
                    ok = ds4_gpu_compressor_prefill_tensor(layer_index_comp_cache_ensure(g, il),
                                                             layer_index_state_kv_ensure(g, il),
                                                             layer_index_state_score_ensure(g, il),
                                                             g->batch_comp_kv,
                                                             g->batch_comp_sc,
    ...
    if (ok && !raw_batch_attention && !batch_attention_done) {
        ...
        if (raw_prefix_tokens < n_tokens) {
            for (uint32_t t = raw_prefix_tokens; ok && t < n_tokens; t++) {
                ...
                if (ok && !zero_prefix) {
                    ok = ds4_gpu_store_raw_kv_tensor(layer_raw_cache_ensure(g, il),
                                                       kv_cache_view,
                                                       g->raw_cap,
                                                       pos % g->raw_cap,
                                                       DS4_N_HEAD_DIM) != 0;
                ...
                if (ok && comp_mask != NULL && n_selected != 0) {
                    ...
                    ok = ds4_gpu_attention_decode_heads_tensor(heads_view,
                                                                 ...
                                                                 g->raw_cap,
                                                                 raw_start,
                                                                 cur_comp ? layer_comp_cache_ensure(g, il) : NULL,
                                                                 cur_comp,
                                                                 comp_mask,
                                                                 n_selected,
                                                                 DS4_N_HEAD,
...
static bool metal_graph_reset_prefill_state(ds4_gpu_graph *g) {
    ...
    for (uint32_t il = 0; il < DS4_N_LAYER; il++) {
        ...
        const uint64_t attn_width = (uint64_t)coff * DS4_N_HEAD_DIM;
        const uint64_t attn_rows = (uint64_t)coff * ratio;
        if (!metal_tensor_fill_f32(layer_attn_state_kv_ensure(g, il), 0.0f, attn_width * attn_rows)) return false;
        if (!metal_tensor_fill_f32(layer_attn_state_score_ensure(g, il), DS4_NEG_INF, attn_width * attn_rows)) return false;
        if (ratio == 4) {
            const uint64_t index_width = (uint64_t)coff * DS4_N_INDEXER_HEAD_DIM;
            const uint64_t index_rows = (uint64_t)coff * ratio;
            if (!metal_tensor_fill_f32(layer_index_state_kv_ensure(g, il), 0.0f, index_width * index_rows)) return false;
            if (!metal_tensor_fill_f32(layer_index_state_score_ensure(g, il), DS4_NEG_INF, index_width * index_rows)) return false;
        }
    }
    return true;
}
...
struct ds4_engine {
    ...
    /* Per-layer q_b scratch.  When the batched-decode path collapses the
     * attn_q_b matmul (q_rank -> q_dim, the biggest dense matmul in the
     * layer) across N sessions, each session's g->qr_norm is staged into
     * batched_qr_norm, one batched Q4 matmul writes batched_q, and rows
     * scatter back to per-session g->q for the per-session head_rms_norm
    ...
    /* Per-layer attn_output scratch.  When the batched-decode path
     * collapses the output_a (grouped) + output_b matmuls across N
     * sessions, each session's g->heads is staged into batched_heads,
     * one batched attention_output_q8_batch_tensor call (n_tokens = N,
     * via the Q4 dispatch added inside it) fills batched_attn_low then
     * batched_attn_out, and rows scatter back to per-session g->attn_out
     * for the (still per-session) hc_expand + FFN HC pre that follows.
     * Forces the non-fused attn_out_hc code path (fuse_attn_out_hc =
     * false equivalent), so PRE_C3 calls a separate hc_expand. */
    ...
    /* Per-layer routed MoE scratch.  When the batched-decode path
     * collapses the routed MoE across N sessions, per-session ffn_norm
     * is already in batched_ffn_norm (shared with SHARED FFN), router
     * selection runs per-session, then router_selected / router_weights
     * are gathered into batched_router_*, one batched MoE call runs, and
     * batched_routed_out scatters back to per-session routed_out. */
    ds4_gpu_tensor *batched_router_selected;
    ds4_gpu_tensor *batched_router_weights;
    ds4_gpu_tensor *batched_routed_gate;
    ds4_gpu_tensor *batched_routed_up;
    ds4_gpu_tensor *batched_routed_mid;
    ds4_gpu_tensor *batched_routed_down;
    ds4_gpu_tensor *batched_routed_out;
#endif
};
...
/* Per-layer routed MoE scratch.  Uses int32 for router_selected,
 * float for everything else.  batched_ffn_norm is shared with the
 * shared-FFN path and must be ensure'd separately. */
static bool ensure_engine_batched_routed_moe_scratch(ds4_engine *e,
                                                      uint64_t expert_mid_dim) {
    /* router_selected: N_EXPERT_USED int32 per session, stored as
     * sizeof(int32_t) per element in a gpu_tensor whose ->bytes
     * covers DS4_BATCH_MAX * N_EXPERT_USED * 4.  Use ensure_engine_scratch
     * with floats-per-row = ceil(N_EXPERT_USED * 4 / 4) = N_EXPERT_USED. */
    if (!e->batched_router_selected) {
        e->batched_router_selected = ds4_gpu_tensor_alloc(
            (uint64_t)DS4_BATCH_MAX * DS4_N_EXPERT_USED * sizeof(int32_t));
        if (!e->batched_router_selected) return false;
        (void)ds4_gpu_tensor_clear(e->batched_router_selected);
    }
    return ensure_engine_scratch(&e->batched_router_weights, DS4_N_EXPERT_USED)
        && ensure_engine_scratch(&e->batched_routed_gate,
                                 (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim)
        && ensure_engine_scratch(&e->batched_routed_up,
                                 (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim)
        && ensure_engine_scratch(&e->batched_routed_mid,
                                 (uint64_t)DS4_N_EXPERT_USED * expert_mid_dim)
        && ensure_engine_scratch(&e->batched_routed_down,
                                 (uint64_t)DS4_N_EXPERT_USED * DS4_N_EMBD)
        && ensure_engine_scratch(&e->batched_routed_out, DS4_N_EMBD);
}

/* Per-layer q_b scratch.  q_rank = DS4_N_LORA_Q for every layer in DS4,
 * so the qr_norm staging buffer can be sized once at first call.  q_dim
 * = DS4_N_HEAD * DS4_N_HEAD_DIM, also constant. */
 ...
 * = n_groups * rank where n_groups = N_OUT_GROUP = 8 and rank = N_LORA_O
 * = 1024.  All constants across layers in DS4. */
...
static bool metal_graph_encode_q_b_batched(
    ...
    if (ok) {
        if (use_q8)
            ok = ds4_gpu_matmul_q8_0_tensor(
                    ...
                    layer->attn_q_b->abs_offset,
                    q_rank, q_dim,
                    e->batched_qr_norm, (uint64_t)n_active) != 0;
        else
            ok = ds4_gpu_matmul_q4_0_batch_warp_tensor(
                    e->batched_q,
                    e->model.map, e->model.size,
    ...
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(sessions[i]->graph.q, 0,
                                       e->batched_q,
...
static bool metal_graph_encode_qkv_batched(
    ...
    /* Scatter-completion fence: see metal_graph_encode_q_b_batched note. */
    if (ok && n_active > 1) ok = ds4_gpu_synchronize() != 0;
    return ok;
}
...
/* Batched attn_output: stage N sessions' g->heads into engine
 * batched_heads, run one ds4_gpu_attention_output_q8_batch_tensor call
 * with n_tokens = N (which internally fans out to the Q4 grouped
 * output_a + Q4 batched output_b paths via the dispatcher additions in
 * attention_output_q8_batch_tensor), then scatter rows of batched_attn_out
 * back to per-session g->attn_out.  Caller follows with PRE_C3 per
 * session, which runs the (non-fused) hc_expand + residual + split
 * combine over the scattered per-session attn_out.
 *
 * Returns false on any failure -- caller falls back to per-session
 * phased(PRE_C2). */
static bool metal_graph_encode_attn_output_batched(
        ds4_engine              *e,
        ds4_session            **sessions,
        int                      n_active,
        const ds4_layer_weights *layer) {
    ...
    if (!ensure_engine_batched_attn_output_scratch(e, q_dim, low_dim)) return false;
    bool ok = true;
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(e->batched_heads,
                                       (uint64_t)i * q_dim * sizeof(float),
                                       sessions[i]->graph.heads, 0,
                                       q_dim * sizeof(float)) != 0;
    }
    if (ok) ok = ds4_gpu_attention_output_q8_batch_tensor(
                    ...
                    (uint32_t)n_active) != 0;
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(sessions[i]->graph.attn_out, 0,
                                       e->batched_attn_out,
                                       (uint64_t)i * DS4_N_EMBD * sizeof(float),
                                       (uint64_t)DS4_N_EMBD * sizeof(float)) != 0;
    }
 ...
 * n_index_comp > indexer top_k AND comp_selected is populated, use the
 * indexed multi-session kernel; otherwise fall through to the non-indexed
 * multi-session kernel (which is what ratio==0/2 layers always use).
 * If any session does not qualify for indexed at a ratio==4 layer, fall
 * back to the per-session phased path (returns false). */
static bool metal_graph_encode_decode_attention_multi_session(
        ds4_engine              *e,
        ds4_session            **sessions,
        int                      n_active,
        const ds4_layer_weights *layer,
        uint32_t                 il) {
    if (n_active <= 0 || n_active > 64) return false;
    const uint64_t q_dim = (uint64_t)DS4_N_HEAD * DS4_N_HEAD_DIM;
    if (!ensure_engine_scratch(&e->batched_q,     q_dim) ||
        !ensure_engine_scratch(&e->batched_heads, q_dim)) {
        return false;
    }
    /* Stage post-rope g->q into engine batched_q. */
    bool ok = true;
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(e->batched_q,
                                       (uint64_t)i * q_dim * sizeof(float),
                                       sessions[i]->graph.q, 0,
                                       q_dim * sizeof(float)) != 0;
    }
    if (!ok) return false;

    /* Gather per-session attention args.  Per-session pos / n_raw /
     * raw_start / n_comp etc. all derive from the session's graph and the
     * current decode position (every batched-decode dispatch produces one
     * token per session).  comp_kv and topk are present only for
     * compressed layers; the topk path additionally requires the indexer
     * to have selected enough rows. */
    const ds4_gpu_tensor *raw_kv[64];
    const ds4_gpu_tensor *comp_kv[64];
    const ds4_gpu_tensor *topk[64];
    uint32_t n_raw_arr[64];
    uint32_t raw_start_arr[64];
    uint32_t n_comp_arr[64];
    uint32_t pos0_arr[64];

    const uint32_t ratio = ds4_layer_compress_ratio(il);
    const uint32_t raw_window = sessions[0]->graph.raw_window;
    const uint32_t raw_cap = sessions[0]->graph.raw_cap;
    const uint32_t decode_top_k = metal_graph_decode_indexer_top_k(&sessions[0]->graph);

    bool use_indexed = (ratio == 4u);
    for (int i = 0; i < n_active; i++) {
        ds4_gpu_graph *g = &sessions[i]->graph;
        const uint32_t pos = ds4_session_pos(sessions[i]);
        const uint32_t n_raw = metal_graph_raw_span_for_batch(g, pos, 1);
        raw_kv[i]    = layer_raw_cache_ensure(g, il);
        if (!raw_kv[i]) return false;
        n_raw_arr[i] = n_raw;
        raw_start_arr[i] = metal_graph_raw_start_for_span(g, pos, n_raw);
        pos0_arr[i]  = pos;
        if (ratio != 0u) {
            n_comp_arr[i] = g->layer_n_comp[il];
            /* Defensive: pass raw_kv as comp_kv fallback when n_comp == 0
             * to mirror single's decode_heads wrapper (ds4_cuda.cu:8529).
             * The CUDA-side wrapper short-circuits on `n_comp[i] && ...`
             * and always writes NULL into the descriptor when n_comp == 0,
             * so this fallback is a no-op as long as the wrapper retains
             * that short-circuit.  Kept for code clarity and to keep the
             * helper-level invariant `comp_kv[i] != NULL`. */
            comp_kv[i] = (n_comp_arr[i] > 0) ? layer_comp_cache_ensure(g, il) : raw_kv[i];
            if (use_indexed) {
                /* Indexed path requires the indexer to have selected
                 * top_k rows for THIS session.  If even one session
                 * has fewer compressed rows than top_k, the indexer
                 * fallback wasn't taken and comp_selected may be
                 * stale -- bail to the per-session fallback. */
                if (g->layer_n_index_comp[il] <= decode_top_k || !g->comp_selected) {
                    use_indexed = false;
                } else {
                    topk[i] = g->comp_selected;
                }
            } else {
                topk[i] = NULL;
            }
            ...
            n_comp_arr[i] = 0;
            comp_kv[i]    = raw_kv[i];  /* defensive non-NULL, see comment above */
            topk[i]       = NULL;
        }
    }

    /* sinks_offset is identical across sessions (same model weights). */
    const uint64_t sinks_offset = layer->attn_sinks->abs_offset;

    if (use_indexed) {
        /* Indexed wrapper passes real window+ratio to its kernel
         * (line 9008-9024 in ds4_cuda.cu); mirror that here. */
        ok = ds4_gpu_attention_indexed_mixed_decode_multi_session_tensor(
                e->batched_heads,
                e->model.map, e->model.size,
                sinks_offset,
                e->batched_q,
                raw_kv, comp_kv, topk,
                n_raw_arr, raw_start_arr, n_comp_arr, pos0_arr,
                (uint32_t)n_active, raw_cap,
    ...
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(sessions[i]->graph.heads, 0,
                                       e->batched_heads,
                                       (uint64_t)i * q_dim * sizeof(float),
                                       q_dim * sizeof(float)) != 0;
    }
...
/* Batched routed MoE.  Expects PRE_C3 to have already run per-session
 * (HC expand, ffn_norm, router selection) so each session's router_selected,
 * router_weights, and ffn_norm are populated.  Gathers those into engine
 * scratch, calls ds4_gpu_routed_moe_batch_tensor(n_tokens = n_active),
 * and scatters batched_routed_out back to per-session routed_out.
 *
 * The win: expert weights read once from HBM per unique (expert, layer)
 * instead of once per (session, expert, layer); with N=8 and 6 experts
 * per session, birthday-paradox overlap means ~7% fewer expert weight
 * reads, and the batch matmul kernel can issue wider grids. */
static bool metal_graph_encode_routed_moe_batched(
        ds4_engine              *e,
        ds4_session            **sessions,
        int                      n_active,
        const ds4_layer_weights *layer) {
    ...
    const uint64_t down_row_bytes = routed_expert_row_bytes(layer->ffn_down_exps);
    const uint64_t down_expert_bytes = routed_out_dim * down_row_bytes;
    if (!ensure_engine_batched_routed_moe_scratch(e, expert_mid_dim)) return false;
    /* batched_ffn_norm is shared with the SHARED FFN path; ensure it. */
    if (!ensure_engine_batched_ffn_scratch(e, (uint64_t)layer->ffn_gate_shexp->dim[1]))
        return false;
    bool ok = true;
    /* Gather per-session ffn_norm, router_selected, router_weights. */
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(e->batched_ffn_norm,
                                       (uint64_t)i * DS4_N_EMBD * sizeof(float),
                                       sessions[i]->graph.ffn_norm, 0,
    ...
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(e->batched_router_selected,
                                       (uint64_t)i * DS4_N_EXPERT_USED * sizeof(int32_t),
                                       sessions[i]->graph.router_selected, 0,
                                       (uint64_t)DS4_N_EXPERT_USED * sizeof(int32_t)) != 0;
    }
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(e->batched_router_weights,
                                       (uint64_t)i * DS4_N_EXPERT_USED * sizeof(float),
                                       sessions[i]->graph.router_weights, 0,
                                       (uint64_t)DS4_N_EXPERT_USED * sizeof(float)) != 0;
    }
    if (ok) {
        bool mid_f16 = false;
        ok = ds4_gpu_routed_moe_batch_tensor(e->batched_routed_out,
                                              e->batched_routed_gate,
                                              e->batched_routed_up,
                                              e->batched_routed_mid,
                                              e->batched_routed_down,
                                              e->model.map, e->model.size,
                                              layer->ffn_gate_exps->abs_offset,
                                              layer->ffn_up_exps->abs_offset,
                                              layer->ffn_down_exps->abs_offset,
                                              ...
                                              (uint32_t)down_in_dim,
                                              (uint32_t)routed_out_dim,
                                              e->batched_router_selected,
                                              e->batched_router_weights,
                                              DS4_N_EXPERT_USED,
                                              DS4_SWIGLU_CLAMP_EXP,
                                              e->batched_ffn_norm,
                                              (uint32_t)n_active,
    ...
    /* Scatter routed_out back to per-session. */
    for (int i = 0; ok && i < n_active; i++) {
        ok = ds4_gpu_tensor_copy_async(sessions[i]->graph.routed_out, 0,
                                       e->batched_routed_out,
                                       (uint64_t)i * DS4_N_EMBD * sizeof(float),
                                       (uint64_t)DS4_N_EMBD * sizeof(float)) != 0;
    }
    return ok;
...
static bool metal_graph_encode_token_raw_swa_batched_n_sessions(
        ...
        bool           want_batched_attn_output,
        bool           want_batched_decode_attention,
        bool           want_batched_routed_moe,
        bool           need_logits) {
    ...
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        const ds4_layer_weights *layer = &weights->layer[il];
        /* PRE phase, with up to three batched substitutions:
         *   want_batched_qkv          replaces PRE_A2 (metal_graph_encode_qkv_batched)
         *   want_batched_q_b          replaces PRE_B  (metal_graph_encode_q_b_batched)
         *   want_batched_attn_output  replaces PRE_C2 (metal_graph_encode_attn_output_batched)
         * Linear data dependency along PRE_A1 -> [PRE_A2 | qkv] -> [PRE_B | q_b]
         * -> PRE_C1 -> [PRE_C2 | attn_output] -> PRE_C3.  Run as a
         * sequence of per-session phased() loops with the engine-level
         * batched calls inserted at the right boundaries.  Each batched
         * call records used_batched_<X>; if it failed, the corresponding
         * fallback PRE_<X> is added to the next per-session pass so g
         * stays internally consistent. */
        bool used_batched_qkv         = false;
        bool used_batched_q_b         = false;
        bool used_batched_attn_output = false;
        ...
        /* Pass 2: run the per-session phases between the
         * (qkv | q_b) boundary and the attn_output boundary that
         * weren't in pass1.  Includes fallbacks if a batched call
         ...
         * runs in pass2b below.  If the multi-session call fails for
         * this layer (e.g. one session does not qualify for the indexed
         * path at a ratio==4 layer), pass2b falls back to the full
         * PRE_C1_ATTN + PRE_C1_POST_ATTN per-session. */
        bool used_batched_decode_attention = false;
        uint32_t pass2 = 0;
        if (want_batched_qkv && !used_batched_qkv) pass2 |= DS4_DECODE_LAYER_PRE_A2;
        if (want_batched_qkv || want_batched_q_b) {
            if (!used_batched_q_b) pass2 |= DS4_DECODE_LAYER_PRE_B;
            if (want_batched_decode_attention) {
                pass2 |= DS4_DECODE_LAYER_PRE_C1_PRE_ATTN;
            } else {
                pass2 |= DS4_DECODE_LAYER_PRE_C1;
            }
            if (!want_batched_attn_output) {
                pass2 |= DS4_DECODE_LAYER_PRE_C2 | DS4_DECODE_LAYER_PRE_C3;
            }
        }
        if (ok && pass2 != 0) {
        ...
        if (ok && want_batched_decode_attention) {
            if (metal_graph_encode_decode_attention_multi_session(engine, sessions,
                                                                  n_active, layer, il)) {
                used_batched_decode_attention = true;
            }
        }
        /* Pass 2b: PRE_C1_POST_ATTN (rope-back on heads) when batched
         * decode attention is active.  If the batched call failed we
         * also need to run PRE_C1_ATTN per-session here as a fallback. */
        if (ok && want_batched_decode_attention) {
            uint32_t pass2b = DS4_DECODE_LAYER_PRE_C1_POST_ATTN;
            if (!used_batched_decode_attention) pass2b |= DS4_DECODE_LAYER_PRE_C1_ATTN;
            ok = metal_graph_encode_phased_per_session(sessions, tokens,
                                                      n_active, model, layer,
                                                      il, pass2b);
        }
        ...
        if (ok && want_batched_attn_output) {
            if (metal_graph_encode_attn_output_batched(engine, sessions, n_active, layer)) {
                used_batched_attn_output = true;
            }
        }

        /* Pass 3: PRE_C3 (plus PRE_C2 fallback if the batched
         * attn_output call failed).  Only runs when attn_output was
         * batched-substituted; otherwise pass1 or pass2 already ran
         * PRE_C2 + PRE_C3.
         *
         * When want_batched_routed_moe is set, run only PRE_C3_ROUTER
         * per-session (HC expand + ffn_norm + router select), then
         * substitute PRE_C3_MOE with the batched MoE call below. */
        if (ok && want_batched_attn_output) {
            uint32_t pass3 = want_batched_routed_moe
                ? DS4_DECODE_LAYER_PRE_C3_ROUTER
                : DS4_DECODE_LAYER_PRE_C3;
            if (!used_batched_attn_output) pass3 |= DS4_DECODE_LAYER_PRE_C2;
            ok = metal_graph_encode_phased_per_session(sessions, tokens,
                                                      n_active, model, layer,
                                                      il, pass3);
        }
        bool used_batched_routed_moe = false;
        if (ok && want_batched_routed_moe) {
            if (metal_graph_encode_routed_moe_batched(engine, sessions,
                                                      n_active, layer)) {
                used_batched_routed_moe = true;
            }
        }
        /* If batched MoE was requested but failed, fall back to
         * per-session PRE_C3_MOE. */
        if (ok && want_batched_routed_moe && !used_batched_routed_moe) {
            ok = metal_graph_encode_phased_per_session(sessions, tokens,
                                                      n_active, model, layer,
                                                      il, DS4_DECODE_LAYER_PRE_C3_MOE);
        }
...
static bool spec_frontier_snapshot(ds4_spec_frontier *f, ds4_session *s) {
    ...
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        ...
        const uint32_t ratio = ds4_layer_compress_ratio(il);
        if (ratio == 0) continue;
        const uint64_t ab = ds4_gpu_tensor_bytes(layer_attn_state_kv_ensure(g, il));
        ok = ds4_gpu_tensor_copy(g->spec_attn_state_kv[il], 0,
                                   layer_attn_state_kv_ensure(g, il), 0, ab) != 0 &&
             ds4_gpu_tensor_copy(g->spec_attn_state_score[il], 0,
                                   layer_attn_state_score_ensure(g, il), 0, ab) != 0;
        if (ratio == 4) {
            const uint64_t ib = ds4_gpu_tensor_bytes(layer_index_state_kv_ensure(g, il));
            ok = ok &&
                 ds4_gpu_tensor_copy(g->spec_index_state_kv[il], 0,
                                       layer_index_state_kv_ensure(g, il), 0, ib) != 0 &&
                 ds4_gpu_tensor_copy(g->spec_index_state_score[il], 0,
                                       layer_index_state_score_ensure(g, il), 0, ib) != 0;
        }
    }
    if (ok) ok = ds4_gpu_end_commands() != 0;
    else (void)ds4_gpu_synchronize();
...
static bool spec_frontier_restore(ds4_spec_frontier *f, ds4_session *s) {
    ...
    for (uint32_t il = 0; ok && il < DS4_N_LAYER; il++) {
        ...
        if (ratio == 0) continue;
        const uint64_t ab = ds4_gpu_tensor_bytes(layer_attn_state_kv_ensure(g, il));
        ok = ds4_gpu_tensor_copy(layer_attn_state_kv_ensure(g, il), 0,
                                   g->spec_attn_state_kv[il], 0, ab) != 0 &&
             ds4_gpu_tensor_copy(layer_attn_state_score_ensure(g, il), 0,
                                   g->spec_attn_state_score[il], 0, ab) != 0;
        if (ok && ratio == 4) {
            const uint64_t ib = ds4_gpu_tensor_bytes(layer_index_state_kv_ensure(g, il));
            ok = ds4_gpu_tensor_copy(layer_index_state_kv_ensure(g, il), 0,
                                       g->spec_index_state_kv[il], 0, ib) != 0 &&
                 ds4_gpu_tensor_copy(layer_index_state_score_ensure(g, il), 0,
...
void ds4_engine_close(ds4_engine *e) {
    ...
    ds4_gpu_tensor_free(e->batched_heads);
    ds4_gpu_tensor_free(e->batched_attn_low);
    ds4_gpu_tensor_free(e->batched_attn_out);
    ds4_gpu_cleanup();
#endif
...
static void session_clamp_logits_for_sampling(ds4_session *s);

int ds4_session_argmax(ds4_session *s) {
    session_clamp_logits_for_sampling(s);
...
static void session_clamp_logits_for_sampling(ds4_session *s) {
    /* Defense-in-depth: replace non-finite logits with -inf so sampling
     * never picks a NaN/Inf token.  Q4 quantization error can accumulate
     * over long context and produce NaN logits.  Also suppress the BOS
     * token, which is never a valid continuation during generation. */
    for (uint32_t i = 0; i < DS4_N_VOCAB; i++) {
        if (!isfinite(s->logits[i])) s->logits[i] = DS4_NEG_INF;
    }
    if (s->engine && s->engine->vocab.bos_id >= 0 &&
        s->engine->vocab.bos_id < (int)DS4_N_VOCAB) {
        s->logits[s->engine->vocab.bos_id] = DS4_NEG_INF;
    }
}
...
int ds4_session_eval_batched_decode(ds4_batch_slot *slots, int n,
    ...
    int DS4_BATCH_INTERLEAVED_MAX = 8;
    {
        const char *cap_env = getenv("DS4_BATCH_INTERLEAVED_MAX");
        if (cap_env) {
            int v = atoi(cap_env);
            if (v >= 2 && v <= DS4_BATCH_MAX) DS4_BATCH_INTERLEAVED_MAX = v;
        }
    }
    if (gpu_fast && engine && n_active > 1 && n_active <= DS4_BATCH_MAX) {
        ...
        const bool want_batched_decode_attention =
            want_batched_attn_output &&
            (getenv("DS4_CUDA_BATCHED_DECODE_ATTENTION") != NULL);
        /* Batched routed MoE requires batched attn_output (PRE_C3 split
         * only implemented in the pass3 path that fires when attn_output
         * is batched). */
        const bool want_batched_routed_moe =
            want_batched_attn_output &&
            (getenv("DS4_CUDA_BATCHED_ROUTED_MOE") != NULL);
        bool ok = ds4_gpu_begin_commands() != 0;
        if (ok && row_count <= DS4_BATCH_INTERLEAVED_MAX) {
            ok = metal_graph_encode_token_raw_swa_batched_n_sessions(
                    ...
                    want_batched_qkv,
                    want_batched_attn_output,
                    want_batched_decode_attention,
                    want_batched_routed_moe,
