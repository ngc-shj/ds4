 ...
 * the default stream (0), so behavior is identical to pre-graph code.
 * When DS4_CUDA_GRAPH=1 the per-token batched-decode encoder routes
 * launches into a non-default stream that is recording into a CUDA Graph;
 * the captured exec is reused across tokens.  See Phase B kernel
 * refactors that lift per-token scalars into g_batch_args so the captured
 * graph stays structurally identical across tokens. */
...
static int cuda_capture_init(void) {
    ...
    if (g_capture_stream) return 1;
    cudaError_t err = cudaStreamCreate(&g_capture_stream);
    if (err != cudaSuccess) {
        fprintf(stderr, "ds4: cudaStreamCreate(capture) failed: %s; disabling capture\n",
                cudaGetErrorString(err));
        (void)cudaGetLastError();
        g_capture_enabled = 0;
        return 0;
 ...
 *   Total per row: blocks_per_row * 18 bytes.  The split lets a warp's
 *   32 lanes read 32 consecutive scales (64 bytes = 1 cache line) and 32
 *   consecutive 16-byte packed groups (512 bytes = 4 cache lines).
 *
...
/* Cache and lazy-convert Q8_0 weights to Q4_0 in device memory.  Returns
 * pointer to the Q4 buffer or NULL if alloc fails.  Always-on caching is
 * intentional: only invoked when the env var gates the Q4 path. */
 ...
 * g_q4_ranges with cuda_q4_from_q8_ptr; offsets in the model file are
 * unique per tensor so there's no collision. */
...
extern "C" int ds4_gpu_init(void) {
    ...
    g_capture_enabled = (getenv("DS4_CUDA_GRAPH") != NULL);
    g_skip_update = (getenv("DS4_CUDA_GRAPH_SKIP_UPDATE") != NULL);
    return 1;
}
...
extern "C" int ds4_gpu_tensor_fill_f32(ds4_gpu_tensor *tensor, float value, uint64_t count) {
    ...
    if (count == 0) return 1;
    fill_f32_kernel<<<(count + 255u) / 256u, 256, 0, g_kernel_stream>>>((float *)tensor->ptr, count, value);
...
extern "C" int ds4_gpu_begin_commands(void) {
    if (cuda_capture_init() && !g_capture_active) {
        ...
        if (err == cudaSuccess) {
            ...
            if (g_cublas_ready) (void)cublasSetStream(g_cublas, g_kernel_stream);
            g_capture_active = 1;
        } else {
...
extern "C" int ds4_gpu_end_commands(void) {
    if (g_capture_active) {
        ...
        int trace_path = 0;
        const char *trace_env = getenv("DS4_CUDA_GRAPH_TRACE");
        if (g_cached_graph_exec && g_skip_update && g_skip_update_armed) {
            ...
            if (err != cudaSuccess || info.result != cudaGraphExecUpdateSuccess) {
                if (trace_env) {
                    fprintf(stderr, "ds4: ExecUpdate FAIL err=%d info.result=%d -> reinstantiate\n",
                            (int)err, (int)info.result);
                }
                (void)cudaGetLastError();
        ...
        if (!g_cached_graph_exec) {
            ...
            g_skip_update_armed = 0;
            trace_path = trace_path == 3 ? 3 : 4;
        }
        if (trace_env) {
            static const char *names[] = {"none", "skip", "exec_ok", "exec_fail+reinst", "fresh_inst"};
            static int seq = 0;
            seq++;
            if (seq <= 10 || seq % 20 == 0) {
                fprintf(stderr, "ds4: graph seq=%d path=%s token=%u pos=%u\n",
                        seq, names[trace_path],
                        g_host_step_args.token, g_host_step_args.pos);
            }
        }
...
extern "C" int ds4_gpu_cache_q4_0_range(const void *model_map, uint64_t model_size, uint64_t offset, uint64_t bytes, uint64_t in_dim, uint64_t out_dim, const char *label) {
    if (!model_map || bytes == 0) return 1;
    if (offset > model_size || bytes > model_size - offset) return 0;
    if (!cuda_q8_use_dp4a()) return 1;
    if (getenv("DS4_CUDA_Q4_DECODE") == NULL) return 1;
    if (getenv("DS4_CUDA_Q8_NO_Q4") != NULL) return 1;
    if (cuda_q4_from_q8_ptr(model_map, offset, bytes, in_dim, out_dim)) return 1;
    fprintf(stderr, "ds4: CUDA q4 preload skipped %s\n", label ? label : "q8_0");
    return 1;
}
...
extern "C" int ds4_gpu_cache_q4_from_f16_range(const void *model_map, uint64_t model_size, uint64_t offset, uint64_t bytes, uint64_t in_dim, uint64_t out_dim, const char *label) {
    ...
    if (getenv("DS4_CUDA_Q4_DECODE") == NULL) return 1;
    if (getenv("DS4_CUDA_Q8_NO_Q4") != NULL) return 1;
    if (getenv("DS4_CUDA_F16_NO_Q4") != NULL) return 1;
...
/* Q4_0 helpers: nibble unpack + per-block dp4a accumulator. */
__device__ __forceinline__ static int32_t q4_unpack4_to_int32(uint8_t p0, uint8_t p1) {
     ...
     * dp4a consumption.  Each byte contains two signed 4-bit values. */
 ...
 * 32 lanes split the in_dim blocks stride-32 (same shape as the warp8 Q8
 * kernel).  53% of Q8_0's HBM traffic. */
...
__global__ static void shared_gate_up_swiglu_q4_0_preq_warp8_kernel(
    ...
    /* Pass 1: gate matmul -- identical shape to matmul_q4_0_preq_warp8. */
    const __half *gate_scales = (const __half *)(gate_w + row * blocks * 18u);
    const unsigned char *gate_packed = gate_w + row * blocks * 18u + blocks * 2u;
    float gate_acc = 0.0f;
    for (uint64_t b = lane; b < blocks; b += 32u) {
        float scale = __half2float(gate_scales[b]);
        int32_t dot = q4_block_dot(gate_packed + b * 16u, xq + b * 32u);
        gate_acc += scale * xscale[b] * (float)dot;
    }
    gate_acc = warp_sum_f32(gate_acc);

    /* Pass 2: up matmul.  Same shape, separate accumulator -- the
     * compiler cannot reorder FMAs across the warp_sum_f32() above. */
    const __half *up_scales = (const __half *)(up_w + row * blocks * 18u);
    const unsigned char *up_packed = up_w + row * blocks * 18u + blocks * 2u;
    float up_acc = 0.0f;
    for (uint64_t b = lane; b < blocks; b += 32u) {
        float scale = __half2float(up_scales[b]);
        int32_t dot = q4_block_dot(up_packed + b * 16u, xq + b * 32u);
        up_acc += scale * xscale[b] * (float)dot;
    }
...
__global__ static void matmul_q4_0_preq_warp16_kernel(
        float *out,
        const unsigned char *w,
        const int8_t *xq,
        const float *xscale,
        ...
        uint64_t blocks) {
    (void)in_dim;
    const uint64_t row = (uint64_t)blockIdx.x * 16u + (threadIdx.x >> 5u);
    const uint32_t lane = threadIdx.x & 31u;
    if (row >= out_dim) return;
    const __half *scales_row     = (const __half *)(w + row * blocks * 18u);
    const unsigned char *packed_row = w + row * blocks * 18u + blocks * 2u;
...
/* Q4 variant of matmul_q8_0_hc_expand_preq_warp8_kernel.  Post-dot HC-
 * expand stage is identical to the Q8 version. */
...
__global__ static void kv_fp8_store_raw_one_kernel(
    ...
    for (uint32_t off = 0; off < n_nope; off += 64) {
        ...
        if (off + tid < n_nope) {
            ...
            kv[off + tid] = q;
            raw[(uint64_t)row * head_dim + off + tid] = __half2float(__float2half(q));
        }
        __syncthreads();
    }
    if (tid < n_rot) {
        uint32_t d = n_nope + tid;
 ...
 * batched_heads with the same layout; the caller scatters batched_heads back
 * to per-session g->heads after the kernel returns. */
struct attention_session_desc {
    const float *raw_kv;
    const float *comp_kv;
    const int32_t *topk;   /* indexed path only; non-indexed leaves NULL */
    uint32_t n_raw;
    uint32_t raw_start;
    uint32_t n_comp;
    uint32_t pos0;
...
__global__ static void attention_decode_mixed_multi_session_kernel(
        float *batched_heads,
        const float *sinks,
        const float *batched_q,
        const attention_session_desc *descs,
        uint32_t raw_cap,
        uint32_t window,
        uint32_t ratio,
        uint32_t n_head,
    ...
    if (h >= n_head) return;
    const attention_session_desc d = descs[s];
    const float *raw_kv = d.raw_kv;
    const float *comp_kv = d.comp_kv;
    const uint32_t n_raw = d.n_raw;
    const uint32_t raw_start = d.raw_start;
    const uint32_t n_comp = d.n_comp;
    const uint32_t pos0 = d.pos0;
    ...
    if (visible_comp > n_comp) visible_comp = n_comp;
    const float *qh = batched_q + ((uint64_t)s * n_head + h) * head_dim;
    ...
    for (uint32_t r = threadIdx.x; r < raw_count; r += blockDim.x) {
        ...
        float dot = 0.0f;
        for (uint32_t dd = 0; dd < head_dim; dd++) dot += qh[dd] * kvrow[dd];
        scores[r] = dot * scale;
        local_max = fmaxf(local_max, scores[r]);
    }
    for (uint32_t c = threadIdx.x; c < visible_comp; c += blockDim.x) {
    ...
    if (threadIdx.x == 0) denom = partial[0] + expf(sinks[h] - max_s);
    __syncthreads();
    float *oh = batched_heads + ((uint64_t)s * n_head + h) * head_dim;
    if (head_dim == 512u && blockDim.x == 256u) {
        ...
        for (uint32_t dd = threadIdx.x; dd < head_dim; dd += blockDim.x) {
            float acc = 0.0f;
            for (uint32_t r = 0; r < raw_count; r++) acc += raw_kv[(uint64_t)raw_rows[r] * head_dim + dd] * scores[r];
            for (uint32_t c = 0; c < visible_comp; c++) acc += comp_kv[(uint64_t)c * head_dim + dd] * scores[raw_count + c];
            oh[dd] = acc / denom;
        }
    }
}

__global__ static void attention_indexed_mixed_multi_session_kernel(
        float *batched_heads,
        const float *sinks,
        const float *batched_q,
        const attention_session_desc *descs,
    ...
    if (comp_count == 0) {
        ...
        for (uint32_t row0 = 0; row0 < n_score; row0 += 32u) {
            ...
            if (row < n_score) {
                ...
                float dot = 0.0f;
                for (uint32_t dd = qlane; dd < head_dim; dd += 8u) dot += qh[dd] * kvrow[dd];
    ...
    if (head_dim == 512u && blockDim.x == 256u) {
        ...
        for (uint32_t dd = threadIdx.x; dd < head_dim; dd += blockDim.x) {
            float acc = 0.0f;
            for (uint32_t r = 0; r < raw_count; r++) acc += raw_kv[(uint64_t)raw_rows[r] * head_dim + dd] * scores[r];
            for (uint32_t c = 0; c < comp_count; c++) acc += comp_kv[(uint64_t)comp_rows[c] * head_dim + dd] * scores[raw_count + c];
            oh[dd] = acc / denom;
        }
    }
}
...
extern "C" int ds4_gpu_embed_token_hc_tensor(ds4_gpu_tensor *out_hc, const void *model_map, uint64_t model_size, uint64_t weight_offset, uint32_t n_vocab, uint32_t token, uint32_t n_embd, uint32_t n_hc) {
    ...
    if (!wptr) return 0;
    uint32_t n = n_embd * n_hc;
    embed_token_hc_kernel<<<(n + 255) / 256, 256, 0, g_kernel_stream>>>((float *)out_hc->ptr, (const unsigned short *)wptr, token, n_embd, n_hc);
...
static int indexer_scores_launch(
    ...
    if (n_tokens == 1u && head_dim == 128u && n_head == 64u &&
        ...
        indexer_score_one_direct_kernel<<<n_comp, 128, 0, g_kernel_stream>>>((float *)scores->ptr,
                                                         (const float *)q->ptr,
    ...
    if (!g_quality_mode && head_dim == 128u && n_head == 64u &&
        ...
        if (getenv("DS4_CUDA_NO_INDEXER_WMMA128") == NULL) {
            ...
            indexer_scores_wmma128_kernel<<<grid, 256, 0, g_kernel_stream>>>((float *)scores->ptr,
                                                         (const float *)q->ptr,
                                                         (const float *)weights->ptr,
                                                         (const float *)index_comp->ptr,
            ...
            indexer_scores_wmma64_kernel<<<grid, 128, 0, g_kernel_stream>>>((float *)scores->ptr,
                                                        (const float *)q->ptr,
                                                        (const float *)weights->ptr,
                                                        (const float *)index_comp->ptr,
                                                        n_comp, n_tokens, pos0, n_head,
            ...
            indexer_scores_wmma32_kernel<<<grid, 64, 0, g_kernel_stream>>>((float *)scores->ptr,
                                                       (const float *)q->ptr,
                                                       (const float *)weights->ptr,
                                                       (const float *)index_comp->ptr,
                                                       n_comp, n_tokens, pos0, n_head,
...
extern "C" int ds4_gpu_indexer_topk_tensor(
        ...
        if (n_comp == 4096u) {
            ...
            if (attr_err == cudaSuccess && max_optin_smem >= smem) {
                attr_err = cudaFuncSetAttribute(indexer_topk_8192_cub_kernel,
                                                ...
                                                smem);
                if (attr_err == cudaSuccess) {
                    indexer_topk_8192_cub_kernel<<<n_tokens, 512, (size_t)smem, g_kernel_stream>>>((uint32_t *)selected->ptr,
                                                                                 (const float *)scores->ptr,
                                                                                 n_comp, n_tokens, top_k);
    ...
    if (top_k == 512u && n_comp <= 8192u &&
        ...
        indexer_topk_pow2_u16_kernel<8192><<<n_tokens, 1024, 0, g_kernel_stream>>>((uint32_t *)selected->ptr,
                                                               (const float *)scores->ptr,
                                                               n_comp, n_tokens, top_k);
    ...
    if (n_tok == 1) {
        ...
        if (use_dp4a && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
            ...
            const unsigned char *w_q4 = cuda_q4_from_q8_ptr(model_map, weight_offset,
                                                            weight_bytes, in_dim, out_dim);
            if (w_q4) {
                if (getenv("DS4_CUDA_Q4_WARP16") != NULL && out_dim >= 4096u) {
                    matmul_q4_0_preq_warp16_kernel<<<((unsigned)out_dim + 15u) / 16u, 512, 0, g_kernel_stream>>>(
                            (float *)out->ptr,
                            w_q4,
                            xq,
                            xscale,
                            ...
                            out_dim,
                            blocks);
                    return cuda_ok(cudaGetLastError(), "matmul_q4_0 warp16 launch");
                }
...
extern "C" int ds4_gpu_matmul_q8_0_dual_tensor(
        ds4_gpu_tensor *out0,
        ds4_gpu_tensor *out1,
        const void *model_map,
        uint64_t model_size,
    ...
    if (!out0 || !out1 || !x || !model_map ||
        in_dim == 0 || out0_dim == 0 || out1_dim == 0) {
        return 0;
    }
    ...
    const char *w0 = cuda_model_range_ptr(model_map, weight0_offset,
                                          weight0_bytes, "q8_0_dual0");
    const char *w1 = cuda_model_range_ptr(model_map, weight1_offset,
                                          weight1_bytes, "q8_0_dual1");
    if (!w0 || !w1) return 0;

    const uint64_t xq_bytes = blocks * 32u;
    const uint64_t scale_offset = (xq_bytes + 15u) & ~15ull;
    void *tmp = cuda_tmp_alloc(scale_offset + blocks * sizeof(float),
                               "q8_0 dual prequant");
    if (!tmp) return 0;
    int8_t *xq = (int8_t *)tmp;
    float *xscale = (float *)((char *)tmp + scale_offset);
    ...
    if (!cuda_ok(cudaGetLastError(), "matmul_q8_0 dual quantize launch")) return 0;
    ...
    if (use_dp4a && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        ...
        if (w0_q4 && w1_q4) {
            matmul_q4_0_preq_warp8_kernel<<<((unsigned)out0_dim + 7u) / 8u, 256, 0, g_kernel_stream>>>(
                    (float *)out0->ptr,
                    w0_q4,
                    xq,
                    xscale,
                    ...
                    out0_dim,
                    blocks);
            if (!cuda_ok(cudaGetLastError(), "matmul_q4_0 dual0 launch")) return 0;
            matmul_q4_0_preq_warp8_kernel<<<((unsigned)out1_dim + 7u) / 8u, 256, 0, g_kernel_stream>>>(
                    (float *)out1->ptr,
                    w1_q4,
                    xq,
                    xscale,
                    ...
                    out1_dim,
                    blocks);
            return cuda_ok(cudaGetLastError(), "matmul_q4_0 dual1 launch");
        }
    }

    matmul_q8_0_preq_warp8_kernel<<<((unsigned)out0_dim + 7u) / 8u, 256, 0, g_kernel_stream>>>(
            (float *)out0->ptr,
            ...
            blocks,
            use_dp4a);
    if (!cuda_ok(cudaGetLastError(), "matmul_q8_0 dual0 launch")) return 0;
    matmul_q8_0_preq_warp8_kernel<<<((unsigned)out1_dim + 7u) / 8u, 256, 0, g_kernel_stream>>>(
            (float *)out1->ptr,
            ...
            blocks,
            use_dp4a);
    return cuda_ok(cudaGetLastError(), "matmul_q8_0 dual1 launch");
}
...
extern "C" int ds4_gpu_matmul_q4_0_batch_warp_tensor(
    ...
    dim3 bgrid(((unsigned)out_dim + 7u) / 8u, (unsigned)n_tok, 1);
    matmul_q4_0_preq_batch_warp8_kernel<<<bgrid, 256, 0, g_kernel_stream>>>(
            (float *)out->ptr,
            w_q4,
            xq,
            xscale,
...
extern "C" int ds4_gpu_matmul_q8_0_pair_tensor(
    ...
    if (use_dp4a && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        getenv("DS4_CUDA_Q8_NO_Q4") == NULL &&
        getenv("DS4_CUDA_Q8_NO_Q4_GENERIC") == NULL &&
        getenv("DS4_CUDA_Q8_NO_Q4_PAIR") == NULL) {
        const unsigned char *w0_q4 = cuda_q4_from_q8_ptr(model_map, weight0_offset,
                                                          weight0_bytes, in_dim, out0_dim);
        const unsigned char *w1_q4 = cuda_q4_from_q8_ptr(model_map, weight1_offset,
                                                          weight1_bytes, in_dim, out1_dim);
...
extern "C" int ds4_gpu_rope_tail_tensor(ds4_gpu_tensor *x, uint32_t n_tok, uint32_t n_head, uint32_t head_dim, uint32_t n_rot, uint32_t pos0, uint32_t n_ctx_orig, bool inverse, float freq_base, float freq_scale, float ext_factor, float attn_factor, float beta_fast, float beta_slow) {
    ...
    uint32_t pairs = n_tok * n_head * (n_rot / 2);
    rope_tail_kernel<<<(pairs + 255) / 256, 256, 0, g_kernel_stream>>>((float *)x->ptr, n_tok, n_head, head_dim, n_rot, pos0, n_ctx_orig, inverse ? 1 : 0, freq_base, freq_scale, ext_factor, attn_factor, beta_fast, beta_slow);
...
extern "C" int ds4_gpu_kv_fp8_store_raw_tensor(
    ...
    if (getenv("DS4_CUDA_NO_FUSED_FP8_STORE") == NULL) {
        kv_fp8_store_raw_one_kernel<<<1, 64, 0, g_kernel_stream>>>(
                (float *)kv->ptr,
                (float *)raw_cache->ptr,
                raw_cap,
                raw_row,
                head_dim,
                n_rot);
        return cuda_ok(cudaGetLastError(), "kv_fp8_store_raw fused launch");
    }
...
extern "C" int ds4_gpu_store_raw_kv_tensor(ds4_gpu_tensor *raw_cache, const ds4_gpu_tensor *kv, uint32_t raw_cap, uint32_t row, uint32_t head_dim) {
    if (!raw_cache || !kv || raw_cap == 0 ||
        raw_cache->bytes < (uint64_t)raw_cap * head_dim * sizeof(float) ||
        kv->bytes < (uint64_t)head_dim * sizeof(float)) return 0;
    store_raw_kv_batch_kernel<<<(head_dim + 255) / 256, 256, 0, g_kernel_stream>>>((float *)raw_cache->ptr, (const float *)kv->ptr, raw_cap, row, 1, head_dim);
...
extern "C" int ds4_gpu_store_raw_kv_batch_tensor(ds4_gpu_tensor *raw_cache, const ds4_gpu_tensor *kv, uint32_t raw_cap, uint32_t pos0, uint32_t n_tokens, uint32_t head_dim) {
    ...
    uint64_t n = (uint64_t)n_tokens * head_dim;
    store_raw_kv_batch_kernel<<<(n + 255) / 256, 256, 0, g_kernel_stream>>>((float *)raw_cache->ptr, (const float *)kv->ptr, raw_cap, pos0, n_tokens, head_dim);
...
extern "C" int ds4_gpu_compressor_prefill_tensor(
    ...
    if (!cuda_ok(cudaMemsetAsync(state_kv->ptr, 0, (size_t)(state_n * sizeof(float)), g_kernel_stream),
 ...
 * already staged Q into batched_q [n_sessions, n_head * head_dim]. */
extern "C" int ds4_gpu_attention_decode_heads_multi_session_tensor(
        ...
        uint32_t                     n_head,
        uint32_t                     head_dim) {
    if (!batched_heads || !batched_q || !model_map || n_sessions == 0 ||
        !raw_kv || !n_raw || !raw_start || !n_comp || !pos0 ||
        sinks_offset > model_size ||
        (uint64_t)n_head * sizeof(float) > model_size - sinks_offset ||
        batched_heads->bytes < (uint64_t)n_sessions * n_head * head_dim * sizeof(float) ||
        batched_q->bytes      < (uint64_t)n_sessions * n_head * head_dim * sizeof(float)) {
        return 0;
    }
    for (uint32_t i = 0; i < n_sessions; i++) {
        if (!raw_kv[i] || n_raw[i] == 0 || raw_cap < n_raw[i] || raw_start[i] >= raw_cap) return 0;
        if (n_comp[i] != 0 && (!comp_kv || !comp_kv[i])) return 0;
        /* n_comp[i] == 0 case: comp_kv[i] may be NULL (UB toggle test) or
         * raw_kv[i] fallback; the kernel won't dereference when
         * visible_comp == 0 either way. */
    }
    /* Aggregate max(n_comp) so the per-block fixed shared_scores buffer
     * (DS4_CUDA_ATTENTION_SCORE_CAP) is sufficient for every session. */
    uint32_t max_n_score = 0;
    for (uint32_t i = 0; i < n_sessions; i++) {
        uint32_t scored_comp = ratio == 0u ? n_comp[i] :
            (n_comp[i] ? ((pos0[i] + 1u) / ratio) : 0u);
        if (scored_comp > n_comp[i]) scored_comp = n_comp[i];
        uint32_t ns = (n_raw[i] > 256u ? 256u : n_raw[i]) + scored_comp;
        if (ns > max_n_score) max_n_score = ns;
    }
    if (max_n_score > DS4_CUDA_ATTENTION_SCORE_CAP) return 0;
    const float *sinks = (const float *)cuda_model_range_ptr(
            model_map, sinks_offset, (uint64_t)n_head * sizeof(float), "attn_sinks");
    if (!sinks) return 0;
    /* Build host-side descriptor array on the stack (n_sessions <= 64),
     * then memcpyAsync to a small device scratch buffer the kernel reads. */
    if (n_sessions > 64u) return 0;
    attention_session_desc h_descs[64];
    const uint64_t desc_bytes = (uint64_t)n_sessions * sizeof(attention_session_desc);
    for (uint32_t i = 0; i < n_sessions; i++) {
        h_descs[i].raw_kv    = (const float *)raw_kv[i]->ptr;
        h_descs[i].comp_kv   = (n_comp[i] && comp_kv && comp_kv[i]) ? (const float *)comp_kv[i]->ptr : NULL;
        h_descs[i].topk      = NULL;
        h_descs[i].n_raw     = n_raw[i];
        h_descs[i].raw_start = raw_start[i];
        h_descs[i].n_comp    = n_comp[i];
        h_descs[i].pos0      = pos0[i];
    }
    void *d_descs = cuda_tmp_alloc(desc_bytes, "attn multi-session device descs");
    if (!d_descs) return 0;
    if (cudaMemcpyAsync(d_descs, h_descs, desc_bytes, cudaMemcpyHostToDevice, g_kernel_stream) != cudaSuccess) {
        return 0;
    }
    dim3 grid(1, n_head, n_sessions);
    attention_decode_mixed_multi_session_kernel<<<grid, 256, 0, g_kernel_stream>>>(
            (float *)batched_heads->ptr,
            sinks,
            (const float *)batched_q->ptr,
            (const attention_session_desc *)d_descs,
            raw_cap, window, ratio, n_head, head_dim);
    return cuda_ok(cudaGetLastError(), "attention multi-session decode launch");
}

extern "C" int ds4_gpu_attention_indexed_mixed_decode_multi_session_tensor(
        ds4_gpu_tensor              *batched_heads,
        const void                   *model_map,
        uint64_t                      model_size,
        uint64_t                      sinks_offset,
        ...
        uint32_t                     n_head,
        uint32_t                     head_dim) {
    if (!batched_heads || !batched_q || !model_map || n_sessions == 0 || top_k == 0 ||
        !raw_kv || !comp_kv || !topk || !n_raw || !raw_start || !n_comp || !pos0 ||
        sinks_offset > model_size ||
        (uint64_t)n_head * sizeof(float) > model_size - sinks_offset ||
        batched_heads->bytes < (uint64_t)n_sessions * n_head * head_dim * sizeof(float) ||
        batched_q->bytes      < (uint64_t)n_sessions * n_head * head_dim * sizeof(float) ||
    ...
    for (uint32_t i = 0; i < n_sessions; i++) {
        if (!raw_kv[i] || !comp_kv[i] || !topk[i] ||
            n_raw[i] == 0 || raw_cap < n_raw[i] || raw_start[i] >= raw_cap ||
            n_comp[i] == 0) return 0;
    }
    const float *sinks = (const float *)cuda_model_range_ptr(
            model_map, sinks_offset, (uint64_t)n_head * sizeof(float), "attn_sinks");
    ...
    for (uint32_t i = 0; i < n_sessions; i++) {
        h_descs[i].raw_kv    = (const float *)raw_kv[i]->ptr;
        h_descs[i].comp_kv   = (const float *)comp_kv[i]->ptr;
        h_descs[i].topk      = (const int32_t *)topk[i]->ptr;
        h_descs[i].n_raw     = n_raw[i];
        h_descs[i].raw_start = raw_start[i];
        h_descs[i].n_comp    = n_comp[i];
        h_descs[i].pos0      = pos0[i];
    }
    void *d_descs = cuda_tmp_alloc(desc_bytes, "attn idx multi-session device descs");
    if (!d_descs) return 0;
    if (cudaMemcpyAsync(d_descs, h_descs, desc_bytes, cudaMemcpyHostToDevice, g_kernel_stream) != cudaSuccess) {
        return 0;
    }
    dim3 grid(1, n_head, n_sessions);
    attention_indexed_mixed_multi_session_kernel<<<grid, 256, 0, g_kernel_stream>>>(
            (float *)batched_heads->ptr,
            sinks,
            (const float *)batched_q->ptr,
            (const attention_session_desc *)d_descs,
            raw_cap, top_k, window, ratio, n_head, head_dim);
    return cuda_ok(cudaGetLastError(), "attention multi-session indexed launch");
}
...
extern "C" int ds4_gpu_attention_indexed_mixed_batch_heads_tensor(
    ...
    if (n_tokens > 1u && top_k == 512u &&
        ...
        if (!sorted) return 0;
        indexed_topk_sort_512_asc_kernel<<<n_tokens, 512, 0, g_kernel_stream>>>(sorted, topk_ptr, n_tokens);
    ...
    if (n_tokens > 1 && head_dim == 512 && top_k <= 512u &&
        ...
        if (getenv("DS4_CUDA_INDEXED_TWOPASS") == NULL) {
            ...
            attention_indexed_mixed_heads8_online_kernel<8, 16><<<grid, 512, 0, g_kernel_stream>>>((float *)heads->ptr,
                                                                               sinks,
                                                                               (const float *)q->ptr,
                                                                               (const float *)raw_kv->ptr,
...
extern "C" int ds4_gpu_attention_output_low_q8_tensor(
    ...
    if (use_dp4a && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        ...
        if (out_a_q4) {
            ...
            grouped_q4_0_a_fused_quant_warp8_kernel
                    <<<fgrid, 256, (size_t)shared_bytes, g_kernel_stream>>>(
                    (float *)low->ptr,
                    out_a_q4,
                    (const float *)heads->ptr,
                    group_dim,
    ...
    if (use_dp4a && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        ...
        if (out_a_q4) {
            grouped_q4_0_a_preq_warp8_kernel<<<grid_a, 256, 0, g_kernel_stream>>>((float *)low->ptr,
                                                              ...
                                                              1,
                                                              blocks_a);
            return cuda_ok(cudaGetLastError(), "attention_output_low_q4 launch");
        }
    }
    grouped_q8_0_a_preq_warp8_kernel<<<grid_a, 256, 0, g_kernel_stream>>>((float *)low->ptr,
                                                      out_a,
...
extern "C" int ds4_gpu_shared_gate_up_swiglu_q8_0_tensor(
        ...
        uint64_t                out_dim,
        const ds4_gpu_tensor *x) {
    /* Day-10 D8-3 fix: the Q4 pair path through ds4_gpu_matmul_q8_0_pair_tensor
     * (matmul_q4_0_pair_preq_warp8_kernel) silently corrupts logits at long
     * context (>= ~5K decode position) -- bisection over 10-runs/config showed
     * baseline 0/10 clean, Q8_NO_Q4_PAIR=1 5/10 (Q4 disabled, Q8 pair still
     * runs the same broken interleaved structure), DISABLE_SHARED_GATE_UP_PAIR=1
     * 10/10 clean (skip the pair wrapper entirely; use single Q4 x 2).
     * Always take the single-x2 path; the pair-kernel optimization can be
     * re-enabled with DS4_CUDA_ENABLE_SHARED_GATE_UP_PAIR=1 for diagnostics
     * but is not safe at long context. */
    ...
    if (cuda_q8_use_dp4a() && getenv("DS4_CUDA_Q4_DECODE") != NULL &&
        ...
        if (out_dim <= UINT64_MAX / (blocks * 34u)) {
            const uint64_t gate_bytes = out_dim * blocks * 34u;
            const uint64_t up_bytes   = out_dim * blocks * 34u;
            if (gate_offset <= model_size && up_offset <= model_size &&
                gate_bytes <= model_size - gate_offset &&
                up_bytes   <= model_size - up_offset &&
                x->bytes >= in_dim * sizeof(float) &&
                mid->bytes >= out_dim * sizeof(float)) {
                const unsigned char *gate_q4 = cuda_q4_from_q8_ptr(model_map, gate_offset,
                                                                    gate_bytes, in_dim, out_dim);
                const unsigned char *up_q4 = cuda_q4_from_q8_ptr(model_map, up_offset,
                                                                    up_bytes, in_dim, out_dim);
                if (gate_q4 && up_q4) {
                    const uint64_t xq_bytes = blocks * 32u;
                    const uint64_t scale_offset = (xq_bytes + 15u) & ~15ull;
                    void *tmp = cuda_tmp_alloc(scale_offset + blocks * sizeof(float),
                                               "shared_gate_up fused prequant");
                    if (tmp) {
                        int8_t *xq = (int8_t *)tmp;
                        float *xscale = (float *)((char *)tmp + scale_offset);
                        quantize_q8_0_f32_kernel<<<(unsigned)blocks, 32, 0, g_kernel_stream>>>(
                        ...
                        if (cuda_ok(cudaGetLastError(),
                                    "shared_gate_up fused quantize launch")) {
                            shared_gate_up_swiglu_q4_0_preq_warp8_kernel
                                    <<<((unsigned)out_dim + 7u) / 8u, 256, 0, g_kernel_stream>>>(
                                    (float *)mid->ptr,
                                    gate_q4,
                                    up_q4,
                                    xq,
    ...
    if (ds4_gpu_matmul_q8_0_dual_tensor(gate, up,
                                           model_map, model_size,
                                           gate_offset, up_offset,
                                           in_dim, out_dim, out_dim,
                                           x)) {
        return ds4_gpu_swiglu_tensor(mid, gate, up, (uint32_t)out_dim, 10.0f, 1.0f);
    }
    ...
    if (down->bytes >= xq_bytes && gate->bytes >= midq_bytes) {
        ...
        if (ok) {
            ...
            if (ok && sorted_pairs && use_expert_tiles && sorted_offsets && sorted_counts && tile_total && tile_experts && tile_starts) {
                ...
                if (use_decode_lut_gate && q4k_path) {
                    moe_gate_up_mid_decode_q4K_qwarp32_kernel<<<qgrid, 256, 0, g_kernel_stream>>>(
                        (float *)gate->ptr,
                        (float *)up->ptr,
                        (float *)mid->ptr,
                        gate_w,
        ...
        if (ok) {
            ...
            if (use_direct_down_sum6) {
                ...
                if (q4k_path) {
                    moe_down_q4K_sum6_qwarp32_kernel<<<sgrid, 256, 0, g_kernel_stream>>>(
                        (float *)out->ptr,
                        down_w,
                        midq,
                        (const int32_t *)selected->ptr,
