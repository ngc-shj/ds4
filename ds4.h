 ...
 *
 * Dispatches to the per-layer interleaved batched encoder when every
 * session is eligible (GPU backend, same engine, no live checkpoint
 * conflict).  Falls back silently to a sequential loop over
 * ds4_session_sync() when any session is ineligible.  The batched
 * encoder uses a phase split inside metal_graph_encode_layer_attention_batch
 * (analogous to the decode-side DS4_DECODE_LAYER_PRE_* bitmask) plus
 * an outer per-layer interleaved loop driving N sessions in lockstep,
 * with batched substitutes (attn_q_a, qkv, attn_output for prefill)
 * slotting between phases. */
